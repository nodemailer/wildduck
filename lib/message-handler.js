        ...
        this.getMailbox(options, (err, mailbox) => {
            ...
            this.indexer.processContent(id, mimeTree, (err, maildata) => {
                ...
                if (maildata.html) {
                    let start = Date.now();
                    try {
                        message.html = sanitizeHtml(maildata.html.replace(/\r\n/g, '\n')).trim();
                    } catch (E) {
                        console.log('ERROR ERRROR ERRROR')
                        console.log(E);
                        console.log(JSON.stringify(maildata.html));
                    }
                    message.html = message.html.length <= maxTextLength ? message.html : message.html.substr(0, maxTextLength);
                    message.htmlParseTime = Date.now() - start;
                }
