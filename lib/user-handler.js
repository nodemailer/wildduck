...
class UserHandler {
                            ...
                            let checkMasterPassword = next => {
                                let checkAccountPassword = () => {
                                    hashes.compare(password, userData.password || '', (err, success) => {
                                        ...
                                        if (/^\$1\$/.test(userData.password)) {
                                            // needs rehashing

                                            return hashes.hash(password, consts.BCRYPT_ROUNDS, (err, hash) => {
                                                if (err) {
                                                ...
                                                return this.users.collection('users').updateOne(
                                                    ...
                                                    err => {
                                                        if (err) {
                                                            ...
                                                            log.info('DB', 'REHASHED user=%s', userData._id);
                                                        }
    ...
    generateASP(user, data, callback) {
        ...
        hashes.hash(password, consts.BCRYPT_ROUNDS, (err, hash) => {
            if (err) {
            ...
            (err, userData) => {
                ...
                checkAddress(() => {
                    ...
                    let hashPassword = done => {
                        ...
                        hashes.hashes(data.password, consts.BCRYPT_ROUNDS, done);
                    };
    ...
    update(user, data, callback) {
        ...
        let hashPassword = done => {
            ...
            hashes.hash($set.password, consts.BCRYPT_ROUNDS, (err, hash) => {
                if (err) {
                    return done(err);
                }
                $set.password = hash;
        ...
        hashPassword(err => {
            ...
            let verifyExistingPassword = next => {
                this.users.collection('users').findOne({ _id: user }, { projection: { password: true, oldPasswords: true } }, (err, userData) => {
                    if (err) {
                        log.error('DB', 'UPDATEFAIL id=%s error=%s', user, err.message);
                        err.message = 'Database Error, failed to find user';
                        err.code = 'InternalDatabaseError';
