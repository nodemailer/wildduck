...
async function getMailboxCounter(db, mailbox, type) {
    if (typeof mailbox === 'string') {
        mailbox = new ObjectId(mailbox);
    }

    let prefix = type ? type : 'total';
    let sum = await db.redis.get(prefix + ':' + mailbox.toString());
...
async function failAction(request, h, err) {
    let details = Object.fromEntries((err.details || []).map(detail => [detail.context.key, detail.message]));

    // do not log provided values
    ...
    let error = Boom.boomify(new Error('Invalid input'), { statusCode: err.responseCode || 400 });

    if (details && Object.keys(details).length) {
        error.output.payload.fields = details;
    }

    error.output.payload.code = err.code || 'InputValidationError';

    throw error;
}

async function inputValidationError(request, h, key, message) {
    ...
    return failAction(request, h, err);
}
...
async function getKeyInfo(pubKeyArmored) {
    ...
    let { name, address } = getPGPUserId(pubKey);
...
function checkWildcardAddress(address, allowWildcard) {
    let wcpos = address.indexOf('*');

    if (wcpos >= 0) {
        if (!allowWildcard) {
            throw new Error('Address can not contain *');
        }
        ...
        if (address.charAt(0) === '*') {
            ...
            if (partial.indexOf('*') >= 0) {
                throw new Error('Invalid wildcard address, only one wildcard symbol allowed');
            }
            ...
            if (partial.substr(0, partial.indexOf('@')).length > consts.MAX_ALLOWED_WILDCARD_LENGTH) {
                throw new Error('Invalid wildcard address, user part suffix too long');
            }

            // result neewds to be a valid email
            if (!isemail.validate(partial)) {
                throw new Error('Invalid wildcard address, not a valid email');
            }
        }
        ...
        if (wcpos !== address.lastIndexOf('*')) {
            throw new Error('Invalid wildcard address, use "*[suffix]@domain" or "user@*"');
        }
        return true;
    }
    return false;
