...
const locationSchema = Joi.string().max(128).example('US-EAST-1').description('Arbitrary location identifier');

const languageSchema = Joi.string().max(20).example('et').description('Language identifier. Will be used to generate localized special use mailbox folders.');

const suspendedSchema = booleanSchema
    .example(false)
...
const autoreplySchema = Joi.object()
    .keys({
        ...
        start: Joi.date()
            .empty('')
            .allow(false)
            .example('2022-02-23T13:17:04.672Z')
            .description('When is autoreply activated. If the value is `false`, then it is not time limited.'),
        end: Joi.date()
            .empty('')
            .allow(false)
            .example('2022-03-23T13:17:04.672Z')
            .description('When is autoreply deactivated. If the value is `false`, then it is not time limited.'),
        name: Joi.string()
            .allow('')
            .trim()
            .max(128)
            .example('John Smith')
            .description('Optional identity name that is used for the From: header in autoreply message'),
        subject: Joi.string()
            .allow('')
            .trim()
            .max(128)
            .example("I'm on vacation")
            .description('Optional subject line of the autoreply. If not set then the subject is derived from the original message'),
        text: Joi.string()
            .allow('')
            .trim()
            .max(128 * 1024)
