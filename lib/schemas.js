...
const sessSchema = Joi.string().max(255).label('SessionIdentifier').description('Session identifier').example('62a173d53d4048599f85c2e0');
...
const tagValueValidator = () => (value, helpers) => {
    let list;
    ...
    const { error: strError, value: processedValue } = Joi.array().items(Joi.string().max(255)).max(255).validate(list);
    if (strError) {
        throw strError;
    }
