...
const deleteMessages = async (task, data) => {
    ...
    try {
        ...
        log.verbose('Tasks', 'task=user-delete id=%s user=%s message=%s', task._id, data.user, `Marked ${markedAsDeleted} messages for deletion`);
    }

    return markedAsDeleted;
};

const deleteRegistryAddresses = async data => {
    let lastId;
...
const run = async (task, data) => {
    ...
    try {
        let deleted = await deleteRegistryAddresses(data);
