...
module.exports = (server, messageHandler, userCache) => (path, options, session, callback) => {
    server.logger.debug(
        {
            tnx: 'fetch',
            cid: session.id
    ...
    db.database.collection('mailboxes').findOne(
        ...
        (err, mailboxData) => {
            ...
            userCache.get(session.user.id, 'imapMaxDownload', (config.imap.maxDownloadMB || 10) * 1024 * 1024, (err, limit) => {
                if (err) {
                    return callback(err);
                }
