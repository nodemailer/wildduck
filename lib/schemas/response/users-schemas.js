...
const GetUsersResult = Joi.object({
    ...
    tags: Joi.items(Joi.string()).required().description('List of tags associated with the User'),
    targets: Joi.items(Joi.string()).required().description('List of forwarding targets'),
    enabled2fa: Joi.items(Joi.string()).required().description('List of enabled 2FA methods'),
