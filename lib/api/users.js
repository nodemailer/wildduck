...
const { getKeyInfo, escapeRegexStr, uview, formatMetaData, failAction } = require('../tools');
const {
...
module.exports = (server, db, userHandler, settingsHandler) => {
    ...
    server.route({
        ...
        async handler(request) {
            ...
            let requiredTags = (request.query.requiredTags || '').split(',');
                ...
                schema: Joi.object({
                    ...
                    results: Joi.array()
                        .items(
                            Joi.object({
                                id: userIdSchema,
                                username: usernameSchema.description('Username of the user'),
                                name: Joi.string().example('John Smith').description('Name of the user'),
