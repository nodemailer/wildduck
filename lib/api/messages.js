...
module.exports = (db, server, messageHandler, userHandler) => {
    ...
    server.post(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            if (!envelope.to.length && referencedMessage && ['reply', 'replyAll'].includes(result.value.reference.action)) {
                envelope.to = envelope.to.concat(referencedMessage.replyTo || []).concat(referencedMessage.replyCc || []);
    ...
    /**
     * @api {post} /users/:user/mailboxes/:mailbox/messages/:message/submit Submit a Draft Message
    ...
    server.post(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            let queueId = await submitMessage(envelope, rebuilder);
