...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    const addThreadCountersToMessageListUnified = async (users, list) => {
        const threadIdsToCount = list.map(message => message.thread);
        const threadCounts = await db.database
            .collection('messages')
            .aggregate([
    ...
    server.post(
        {
            name: 'messages',
            path: '/users/unified/mailboxes/messages',
            summary: 'List messages across all aliases of a user',
            description: 'List messages across all aliases of a user',
            validationObjs: {
                requestBody: {
                    users: Joi.array().items(
                        Joi.string().hex().lowercase().length(24).required().description('IDs of Users')
                    ),
                    mailboxes: Joi.array().items(
                        Joi.string().hex().lowercase().length(24).optional().description('IDs of Mailboxes')
                    ).optional(),
                    unseen: booleanSchema.description('If true, then returns only unseen messages'),
                    metaData: booleanSchema.default(false).description('If true, then includes metaData in the response'),
                    threadCounters: booleanSchema
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            console.log(result.value.mailboxes);

            let users = result.value.users.map((user_id) => new ObjectId(user_id)) ;
            let mailboxes = result.value.mailboxes.map((mailbox_id) => new ObjectId(mailbox_id));
            let limit = result.value.limit;
            let threadCounters = result.value.threadCounters;
            let page = result.value.page;
            let pageNext = result.value.next;
            ...
            if (threadCounters) {
                listing.results = await addThreadCountersToMessageListUnified(users, listing.results);
            }
