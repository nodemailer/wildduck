...
module.exports = (db, server, messageHandler, userHandler, storageHandler) => {
     ...
     * @apiParam {String} mailbox ID of the Mailbox
     * @apiParam {Number} message ID of the Message
     * @apiParam {Number} The time duration (in milliseconds) to keep the message snoozed for
     *
     * @apiSuccess {Boolean} success Indicates successful response
     * @apiSuccess {Number} id ID of the Message
     * @apiSuccess {String} mailbox ID of the Mailbox
     ...
     *
     * @apiExample {curl} Example usage:
     *     curl -i -XPOST "http://localhost:8080/users/59fc66a03e54454869460e45/mailboxes/59fc66a03e54454869460e46/messages/1/snooze"
     *
     * @apiSuccessExample {json} Success-Response:
     *     HTTP/1.1 200 OK
     *     {
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            try {
                let data = await moveMessage({
                    user,
                    source: { user, mailbox },
                    destination: { user, mailbox: "[Snoozed]/" + mailbox },
                    messageQuery
                });
                info = data.info;
            } catch (err) {
