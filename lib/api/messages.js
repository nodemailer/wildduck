...
module.exports = (db, server, messageHandler) => {
    ...
    /**
     * @api {post} /users/:user/restore Restore archived messages
     ...
     *
     * @apiExample {curl} Restore a Message:
     *     curl -i -XPOST "http://localhost:8080/users/59fc66a03e54454869460e45/restore" \
     *     -H 'Content-type: application/json' \
     *     -d '{
     ...
     */
    server.post(
        { name: 'create_restore_task', path: '/users/:user/restore' },
        tools.asyncifyJson(async (req, res, next) => {
            res.charSet('utf-8');
