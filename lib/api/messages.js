...
module.exports = (db, server, messageHandler, userHandler, storageHandler) => {
    ...
    const putMessageHandler = async (req, res, next) => {
        ...
        const schema = Joi.object().keys({
            ...
            message: Joi.string()
                .regex(/^\d+(,\d+)*$|^\d+:\d+$/i)
                .required(),
            seen: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
            deleted: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
            flagged: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
            draft: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
            expires: Joi.alternatives().try(
                Joi.date(),
                Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']).allow(false)
            ),
    ...
    server.get(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                user: Joi.string().hex().lowercase().length(24).required(),
                mailbox: Joi.string().hex().lowercase().length(24).required(),
                unseen: Joi.boolean().empty('').truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
                metaData: Joi.boolean().empty('').truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']).default(false),
    ...
    server.get(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                ...
                to: Joi.string().trim().empty(''),
                subject: Joi.string().trim().empty(''),
                attachments: Joi.boolean().empty('').truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
                flagged: Joi.boolean().empty('').truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
                unseen: Joi.boolean().empty('').truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
                searchable: Joi.boolean().empty('').truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
    ...
    server.get(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                ...
                replaceCidLinks: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']).default(false),
                markAsSeen: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']).default(false),
                sess: Joi.string().max(255),
                ip: Joi.string().ip({
                    version: ['ipv4', 'ipv6'],
                    cidr: 'forbidden'
     ...
     * @apiParam {String} user ID of the User
     * @apiParam {String} mailbox ID of the Mailbox
     * @apiParam {Boolean} [async=false] Schedule deletion task
     *
     * @apiSuccess {Boolean} success Indicates successful response
    ...
    server.del(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                user: Joi.string().hex().lowercase().length(24).required(),
                mailbox: Joi.string().hex().lowercase().length(24).required(),
                async: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']).default(false),
                skipArchive: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']).default(false),
                sess: Joi.string().max(255),
                ip: Joi.string().ip({
                    version: ['ipv4', 'ipv6'],
                    cidr: 'forbidden'
            ...
            if (result.value.async) {
                ...
                let now = new Date();
                let r = await db.database.collection('tasks').findOneAndUpdate(
                    // NB! not indexed fields
                    { task: 'clear-folder', user, mailbox },
                    {
                        $setOnInsert: {
                            task: 'clear-folder',
                            locked: false,
                            ...
                            user,
                            mailbox,
                            skipArchive: result.value.skipArchive
                        },
                        $set: {
                            updated: now
                        }
                ...
                res.json({
                    success: true,
                    existing: !(r && r.lastErrorObject && r.lastErrorObject.upserted),
                    scheduled: r && r.value && r.value._id
                });
     ...
     *     }'
     *
     * @apiExample {curl} Upload Raw Message:
     *     curl -i -XPOST "http://localhost:8080/users/5a2f9ca57308fc3a6f5f811d/mailboxes/5a2f9ca57308fc3a6f5f811e/messages" \
    ...
    server.post(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                ...
                mailbox: Joi.string().hex().lowercase().length(24).required(),
                date: Joi.date(),
                unseen: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']).default(false),
                flagged: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']).default(false),
                draft: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']).default(false),
                ...
                reference: Joi.object().keys({
                    ...
                    attachments: Joi.alternatives().try(
                        Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
                        Joi.array()
                            .items(
                                Joi.string()
                                    .regex(/^ATT\d+$/i)
    ...
    server.post(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                ...
                mailbox: Joi.string().hex().lowercase().length(24).required(),
                message: Joi.number().required(),
                deleteFiles: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
                sess: Joi.string().max(255),
                ip: Joi.string().ip({
                    version: ['ipv4', 'ipv6'],
                    cidr: 'forbidden'
