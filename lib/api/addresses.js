...
module.exports = (db, server, userHandler) => {
    ...
    server.post(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                ...
                address: [Joi.string().email().required(), Joi.string().regex(/^\w+@\*$/, 'special address')],
                name: Joi.string().empty('').trim().max(128),
                main: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
                allowWildcard: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                address: Joi.alternatives()
                    .try(Joi.string().email().required(), Joi.string().regex(/^\w+@\*$/, 'special address'))
                    .required(),
                name: Joi.string().empty('').trim().max(128),
                ...
                autoreply: Joi.object().keys({
                    status: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']).default(true),
    ...
    server.put(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                ...
                autoreply: Joi.object().keys({
                    status: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
                    start: Joi.date().empty('').allow(false),
                    end: Joi.date().empty('').allow(false),
                    name: Joi.string().empty('').trim().max(128),
                    subject: Joi.string().empty('').trim().max(128),
    ...
    server.get(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                address: [Joi.string().hex().lowercase().length(24).required(), Joi.string().email()],
                allowWildcard: Joi.boolean().truthy(['Y', 'true', 'yes', 'on', '1', 1]).falsy(['N', 'false', 'no', 'off', '0', 0, '']),
                sess: Joi.string().max(255),
                ip: Joi.string().ip({
                    version: ['ipv4', 'ipv6'],
