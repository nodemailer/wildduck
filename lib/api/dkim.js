...
module.exports = (db, server) => {
     ...
     * @apiParam {String} [domain] Exact match of a Domain name
    ...
    server.get({ name: 'dkim', path: '/dkim' }, (req, res, next) => {
        ...
        let domain = result.value.domain && tools.normalizeDomain(result.value.domain);
        ...
        db.users.collection('dkim').count(filter, (err, total) => {
            ...
            let opts = {
                limit,
                query: filter,
                paginatedField: domain ? '_id' : 'domain',
                sortAscending: domain ? false : true
            };
    ...
    server.post('/dkim', (req, res, next) => {
        ...
        db.users.collection('dkim').findOne(
            ...
            (err, aliasData) => {
                ...
                if (aliasData) {
                    res.json({
                        error: 'DKIM key for the same selector already exists',
                        code: 'SelectorExists'
                    });
                    return next();
                }
                ...
                db.users.collection('dkim').insertOne(dkimData, (err, r) => {
                    if (err) {
                        res.json({
                            error: 'MongoDB Error: ' + err.message,
                            code: 'InternalDatabaseError'
