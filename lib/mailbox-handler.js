    ...
    create(user, path, opts, callback) {
        this.database.collection('mailboxes').findOne(
            ...
            (err, mailboxData) => {
                ...
                if (mailboxData) {
                    const err = new Error('Mailbox creation failed with code ALREADYEXISTS');
    ...
    del(user, mailbox, callback) {
        this.database.collection('mailboxes').findOne(
            ...
            (err, mailboxData) => {
                ...
                if (!mailboxData) {
                    const err = new Error('Mailbox deletion failed with code NONEXISTENT');
                    err.code = 'NONEXISTENT';
                    err.httpStatus = 404;
                    return callback(err, 'NONEXISTENT');
                }
                if (mailboxData.specialUse || mailboxData.path === 'INBOX' || mailboxData.hidden) {
                    const err = new Error('Mailbox deletion failed with code CANNOT');
                    err.code = 'CANNOT';
                    err.httpStatus = 500;
