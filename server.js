...
MongoClient.connect(config.mongo, (err, database) => {
    ...
    server.addToMailbox = (username, path, meta, date, flags, raw, callback) => {
        server.logger.debug('[%s] Appending message to "%s"', 'API', path);
        ...
        simpleParser(raw, (err, parsed) => {
            ...
            parsed.attachments = parsed.attachments && parsed.attachments.length ? true : false;
                        ...
                        database.collection('messages').insertOne(message, err => {
                            ...
                            server.notifier.addEntries(username, path, {
                                ...
                                let cursor = database.collection('messages').find({
                                    ...
                                    ['uid', -1]
                                ]).skip(config.imap.maxUnflaggedMessages);
